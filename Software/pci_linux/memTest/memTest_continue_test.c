...
main(void)
{
	...
	unsigned short reg_data_hex16;
	unsigned int reg_data_hex32;
	unsigned int old_reg_addr_hex;
	unsigned char temp8;
	...
	for(;;) {
		...
		if(cmd_buf[0] == 'm' || cmd_buf[0] == 'w'){
			...
			reg_data_hex16 = (short)my_atoh(reg_data_str);
			...
			if (!strncmp(cmd_buf, "whoru",5)) {
				...
				do{
					...
					printf("\r\n write reg address : %x",old_reg_addr_hex);
					printf("\r\n write reg data : ");
					gets(cmd_buf);
					temp8 = cmd_buf[0];
					...
					if(temp8 != 0x00){
						...
						reg_data_hex = (unsigned int)my_atoh(reg_data_str);
						printf("\r\n write reg address : %x",old_reg_addr_hex);
						printf("\r\n write reg data : %x",reg_data_hex);
				...
				old_reg_addr_hex = reg_addr_hex;
				reg_data_hex32 = reg_read(reg_addr_hex);
				printf("\r\n read reg address : %x",reg_addr_hex);
				printf("\r\n read reg data : %x",reg_data_hex32);
				do{
					gets(cmd_buf);
					temp8 = cmd_buf[0];
					if(temp8 == 0x00){	//enter key
						old_reg_addr_hex = old_reg_addr_hex + 4;
						reg_data_hex32 = reg_read(old_reg_addr_hex);
						printf("\r\n read reg address : %x",old_reg_addr_hex);
						printf("\r\n read reg data : %x",(unsigned int)reg_data_hex32);
					}
